			...
			var showAppDescription = function(app){
				...
				if(cache.hasOwnProperty(cacheIndex)){
					...
					"success" : function(data){
							var cleaned = '<div class="outputDiv" id="outputDiv">'+cleanAppPage(data) + '</div>';
							$("#apps").append(cleaned);
			...
			* Apps with a lower sortPriority will appear ones with higher or no sortPriority.
			* Note that it allows for < 1 difference in priority, while maintaining convention 
			* of -1/0/+1 return value for comparison.
