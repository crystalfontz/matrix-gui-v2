...
exports.createMenuPage = function(apps, response, submenu, doMenu){
		...
		jsdom.env({
			...
			scripts: [ configs.jqueryUrl ] 
				...
				if(!doMenu){
					...
					var cacheData = JSON.stringify(apps)
					...
					path.exists(appListCache, function(exists){
						if(!exists){			
							fs.writeFile(appListCache, cacheData, function(err){
								if(err){
				...
				/*					
					if(submenu == "" && (!apps[idx].Application.hasOwnProperty("categories") || apps[idx].Application.categories == "")){
						addApp($, window, appContainer , apps[idx]);
					}else if(apps[idx].Application.hasOwnProperty("categories") && mUtils.appInSubmenu(apps[idx].Application.categories, submenu)){
						addApp($, window, appContainer , apps[idx]);
					}
				*/
